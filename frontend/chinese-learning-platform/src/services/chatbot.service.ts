...
export class ChatbotService {
  ...
  static async captureScreenshot(videoId: string, time: number): Promise<{image: string}> {
    const response = await api.post<{image: string}>('/chatbot/screenshot', { videoId, time });
    return response.data;
  }
}